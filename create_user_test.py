...
def test_create_user_15_letter_in_first_name_get_success_response():
    # В переменную user_body сохраняется обновлённое тело запроса с именем "Аа"
    user_body = get_user_body("Aaааааааааааааа")
    # В переменную user_response сохраняется результат запроса на создание пользователя
    user_response = sender_stand_request.post_new_user(user_body)
...
def negative_assert_symbol(first_name):
    # Получаем тело запроса с изменённым firstName
    user_body = get_user_body(first_name)
    ...
    # Проверка, что поле code в json тоже 400
...
# Тест 9. Ошибка 
    # Параметр firstName состоит из слов с цифрой
